...
static void coalesce_free_block(block_info* old_block) {
  ...
  if (new_size != old_size) {
    ...
    *(size_t*) UNSCALED_POINTER_SUB(block_cursor, WORD_SIZE) = new_size | TAG_PRECEDING_USED;
    ...
    /* The block after the coalesced free block now has a free predecessor.
     * Clear its TAG_PRECEDING_USED so that the following block knows the
     * preceding block is free. */
    following_block = (block_info*) UNSCALED_POINTER_ADD(new_block, new_size);
...
void* mm_malloc(size_t size) {
  ...
  /* Find a free block of at least req_size; request more heap if needed. */
  while ((ptr_free_block = search_free_list(req_size)) == NULL) {
  ...
  /* Remove chosen free block from the free list before modifying it. */
  remove_free_block(ptr_free_block);

  if (block_size - req_size >= MIN_BLOCK_SIZE) {
    /* Split: create a remainder free block at the end of the chosen block. */
    remainder_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    /* Set header for remainder and mark that its predecessor is used (temporarily). */
    remainder_block->size_and_tags = (block_size - req_size) | TAG_PRECEDING_USED;
    /* Write footer for remainder block. */
    *((size_t*) UNSCALED_POINTER_ADD(remainder_block, block_size - req_size - WORD_SIZE)) =
        (block_size - req_size) | TAG_PRECEDING_USED;
    /* Insert remainder into free list. */
    insert_free_block(remainder_block);

    /* The block after the remainder now has a free predecessor; clear its tag. */
    following_block = (block_info*) UNSCALED_POINTER_ADD(remainder_block, block_size - req_size);
    following_block->size_and_tags &= ~TAG_PRECEDING_USED;
  } else {
    /* Not enough space to split: allocate the entire free block. */
    req_size = block_size;
    following_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, block_size);
    /* Mark following block as having a used predecessor. */
    following_block->size_and_tags |= TAG_PRECEDING_USED;
  }

  /* Mark the chosen block as used and preserve the preceding-used tag. */
  ptr_free_block->size_and_tags = req_size | preceding_block_use_tag | TAG_USED;
  /* Return pointer to payload (word after header). */
...
void mm_free(void* ptr) {
  ...
  /* Compute pointer to the block header from the user pointer. */
  block_to_free = (block_info*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  payload_size = SIZE(block_to_free->size_and_tags);

  /* Convert block to a free block: set header (keep preceding tag) and write footer. */
  block_to_free->size_and_tags = payload_size | (block_to_free->size_and_tags & TAG_PRECEDING_USED);
  *((size_t*) UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE)) = block_to_free->size_and_tags;

  /* The following block now has a free predecessor; clear its TAG_PRECEDING_USED. */
  following_block = (block_info*) UNSCALED_POINTER_ADD(block_to_free, payload_size);
  following_block->size_and_tags &= ~TAG_PRECEDING_USED;

  /* Insert into free list and coalesce with any adjacent free blocks. */
