...
INTEGER CODING RULES:
  ...
  1. Uses 2s complement, 32-bit representations of integers.  However, you
     are NOT allowed to simply return a constant (eg, "return 4;") from
     problems that ask you to determine the size of a type (eg, "int_size").
 ...
 *   Binary integer operators: &, &&, |, ||, <, >, <<, >>, ==, !=, ^, /, %
 *   Unary integer operators: ~, -
 *   Returning a hard-coded value based on this machine (eg, "return 4")
 */
int int_size() {
  // Subtracts adjacent int addresses as char pointers.
  int int_array[10];
  int* int_ptr1;
  int* int_ptr2;
 ...
 *   Binary integer operators: &, &&, |, ||, <, >, <<, >>, ==, !=, ^, /, %
 *   Unary integer operators: ~, -
 *   Returning a hard-coded value based on this machine (eg, "return 8")
 */
int double_size() {
  // Subtracts adjacent double addresses as char pointers.
 ...
 */
int pointer_size() {
  // Subtracts adjacent pointer addresses as char pointers.
  double* ptr_array[10];
  double** ptr_ptr1;
  double** ptr_ptr2;
...
void swap_ints(int* ptr1, int* ptr2) {
  // Swaps values using a temporary variable and pointer dereferences.
 ...
 */
int change_value() {
  // Move to the sixth element with pointer arithmetic and store/access 351 there with *.	
  int int_array[10];
...
int within_same_block(int* ptr1, int* ptr2) {
  // XOR addresses as longs and check if any bits above the lower 6 digits(63 bits) differ.
  
  long diff = (long) ptr1 ^ (long) ptr2;
  return !(diff >> 6);
}
...
int within_array(int* int_array, int size, int* ptr) {
  // Convert to char pointers and check if ptr is out of bounds by shifting 31 bits to check the 2's complement sign bit. 
  // If either is 1 then ptr is out of bounds, otherwise it's within the array.

  int start = ((char*)ptr - (char*)int_array) >> 31;
  int end = ((char*)(int_array + size) - 1 - (char*)ptr) >> 31;
  return !start * !end;
}
...
int string_length(char* s) {
  // Move forward until the null terminator and count traversed characters.
...
int endian_experiment(int* ptr) {
  // Convert to char pointer and update each byte to each pair of hex digits in 351351 (0x00055F67) in little endian order.
  char* byte_ptr;
...
void selection_sort(int arr[], int arr_length) {
  // Using int pointers, repeatedly find the minimum remaining element in the unsorted section 
  // and swap it into the end of the sorted section.
  int* i;
  int* j;
  int* min_index;

  for (i = arr; i < arr + arr_length - 1; i++) {
	min_index = i;
	for (j = i + 1; j < arr + arr_length; j++) {
		if (*min_index > *j) {
       			min_index = j;
		}
	}
	swap_ints(i, min_index);
  }
}
